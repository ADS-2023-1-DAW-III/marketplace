...
export default function Sidebar() {
  ...
  const getLinkClasses = (path: string, isSubItem = false) => {
    return `flex items-center font-poppins font-medium text-lg py-3 transition-colors duration-500 ${
      isSubItem ? "pl-14" : "pl-10" 
    } ${
      isActive(path)
        ? "bg-white text-[#307B8E] stroke-[#307B8E]" 
        : "hover:bg-white hover:text-[#307B8E] hover:stroke-[#307B8E] stroke-white text-white" 
    }`;
  };
  ...
  return (
    <Sheet>
      ...
      <SheetContent side="left" className="bg-[#307B8E] text-white">
        <SheetHeader>
          <SheetTitle className="text-3xl mt-10 mr-auto ml-auto font-bold text-white cursor-default">
        ...
        <nav className="flex flex-col gap-4 mx-2">
          ...
          {/* Item "Serviços prestados" que agora será um toggle */}
          <div
            onClick={toggleServicesSubmenu}
            className={`flex items-center justify-between cursor-pointer font-poppins font-medium text-lg py-3 pl-10 transition-colors duration-500 
            ${
              isServicesOpen ||
              isActive("/servicos_prestados") ||
              isActive("/negociacoes")
                ? "bg-white text-[#307B8E] stroke-[#307B8E]" 
                : "hover:bg-white hover:text-[#307B8E] hover:stroke-[#307B8E] stroke-white text-white"
            }
            `}
          >
            ...
            {isServicesOpen ? (
              <ChevronUp size={20} className="mr-5" />
            ) : (
              <ChevronDown size={20} className="mr-5" />
            )}
          </div>
          {isServicesOpen && (
            <div className="flex flex-col">
              <Link
                ...
                className={getLinkClasses("/meus_servicos", true)}
              >
                Meus serviços
              </Link>
              <Link
                to="/negociacoes"
                className={getLinkClasses("/negociacoes", true)} 
              >
                Negociações
              </Link>
            </div>
          ...
          <Link to="/contratados" className={getLinkClasses("/contratados")}>
            <svg
              width="22"
              height="22"
              viewBox="0 0 24 24"
          ...
          <Link
            to="/historico_pagamento"
            className={getLinkClasses("/historico_pagamento")}
          >
            <svg
              width="22"
              height="22"
            ...
            >
              ...
              />
              <path
                d="M2 8V9C2 9.53043 2.21071 10.0391 2.58579 10.4142C2.96086 10.7893 3.46957 11 4 11H5"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
              />
